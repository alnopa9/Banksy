...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="banksyInputColl" as="document-node()+" select="collection('../xml/xml/exhibits/?select=*.xml')"/>
    <xsl:template match="/">
        <xsl:for-each select="$banksyInputColl//root">
            <xsl:variable name="filename">
                <xsl:value-of select="replace(base-uri(), '\.xml', '.html') ! tokenize(., '/')[last()]"/>
            </xsl:variable>
            <xsl:result-document method="xhtml" indent="yes" href="../site/html/xslt_gallery_pages/Exhibits/{$filename}">
                <html>
                    <head>
                                ...
                                <div class="single-work">
                                    <xsl:if test="descendant::sourceDesc//size[@orientation='portrait']">
                                        <div class="gallery-page-img" id="portrait">
                                            <xsl:text>&#10;</xsl:text>
                                            <img src="../../../img/exhibits/{replace(. ! base-uri() ! tokenize(., '/')[last()], '\.xml', '')}/{(descendant::body//img/@src)[1]}" alt="{(descendant::body//img/@alt/string())[1]}"/>
                                            <xsl:text>&#10;</xsl:text>
    ...
    <xsl:template match="descendant::img[parent::p][position() mod 3 = 1]" mode="column1">
        <div class="thumbnail-hover">
            <img src="../../../img/exhibits/{replace(. ! base-uri() ! tokenize(., '/')[last()], '\.xml', '')}/{@src/string()}" alt="{current()/@alt/string()}"/>
        </div>
    </xsl:template>
